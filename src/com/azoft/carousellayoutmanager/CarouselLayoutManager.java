...
public class CarouselLayoutManager extends RecyclerView.LayoutManager/*implements RecyclerView.SmoothScroller.ScrollVectorProvider*/ {
    ...
    public CarouselLayoutManager(Context context, final int orientation) {
        this(context, orientation, CIRCLE_LAYOUT);
    }
     ...
     */
    @SuppressWarnings("unused")
    public CarouselLayoutManager(Context context, final int orientation, final boolean circleLayout) {
    ...
    public void smoothScrollToPosition(@NonNull final RecyclerView recyclerView, @NonNull final RecyclerView.State state, final int position) {
        final LinearSmoothScroller linearSmoothScroller = new LinearSmoothScroller(recyclerView.getContext()) {
            ...
            protected float calculateSpeedPerPixel(DisplayMetrics displayMetrics) {
                return getCalculateSpeed() / displayMetrics.density;
                //返回滑动一个pixel需要多少毫秒
            }
        };
        linearSmoothScroller.setTargetPosition(position);
        int count = position - getCenterItemPosition();
        if(Math.abs(count) > 5){
            if(position - 3 < 0 || count < 0){
    ...
    private static float mCalculateSpeed = 0.8f;
    public void setCalculateSpeed(float speed){
        this.mCalculateSpeed = speed;
    }

    public float getCalculateSpeed(){
        if(mContxt != null){
            return mContxt.getResources().getDisplayMetrics().density * mCalculateSpeed;
        }else{
            return -1;
        }
    }
    ...
    private void fillChildItem(final int start, final int top, final int end, final int bottom, @NonNull final LayoutOrder layoutOrder, @NonNull final RecyclerView.Recycler recycler, final int i, final boolean childMeasuringNeeded) {
        ...
        if (null != mViewPostLayout) {
            transformation = mViewPostLayout.transformChild(view, layoutOrder.mItemPositionDiff, mOrientation, mLayoutHelper.mMaxVisibleItems);
        }
    ...
    protected double convertItemPositionDiffToSmoothPositionDiff(final float itemPositionDiff) {
        ...
        // update this for used same interval
        if (true) {
            return absIemPositionDiff / mLayoutHelper.mMaxVisibleItems;
        }
    ...
    public interface PostLayoutListener {
         ...
         * @param maxVisibleItems          layoutHelper mMaxVisibleItems
         */
        ItemTransformation transformChild(@NonNull final View child, final float itemPositionToCenterDiff, final int orientation, final int maxVisibleItems);
    }
